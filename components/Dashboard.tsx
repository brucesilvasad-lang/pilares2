...
const StatCard: React.FC<{ title: string; value: string; color: string; icon: React.ReactNode }> = ({ title, value, color, icon }) => (
    <div className={`bg-brand-white p-6 rounded-xl shadow-lg flex items-center space-x-4 border-l-4 ${color}`}>
        <div className="text-3xl">{icon}</div>
        <div>
            <p className="text-sm font-medium text-brand-gray">{title}</p>
            <p className="text-xl sm:text-2xl font-bold text-brand-darkgray">{value}</p>
        </div>
    </div>
);
...
const Dashboard: React.FC<DashboardProps> = ({ schedule, expenses, services }) => {
  ...
  return (
    <div className="max-w-7xl mx-auto">
      <div className="mb-8">
        <h2 className="text-2xl sm:text-3xl font-bold text-brand-darkgray">Dashboard</h2>
        <p className="text-brand-gray">Resumo financeiro e de atendimentos do dia.</p>
      </div>
      ...
      <div>
        <h3 className="text-2xl font-bold text-brand-darkgray mb-4">Estatísticas de Atendimento</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
            <div className="bg-brand-white p-6 rounded-xl shadow-lg text-center">
                <p className="text-3xl sm:text-4xl font-bold text-green-600">{present}</p>
                <p className="font-semibold text-brand-gray">Presentes</p>
            </div>
            <div className="bg-brand-white p-6 rounded-xl shadow-lg text-center">
                <p className="text-3xl sm:text-4xl font-bold text-red-600">{absent}</p>
                <p className="font-semibold text-brand-gray">Faltas</p>
            </div>
            <div className="bg-brand-white p-6 rounded-xl shadow-lg text-center">
                <p className="text-3xl sm:text-4xl font-bold text-yellow-600">{vacant}</p>
                <p className="font-semibold text-brand-gray">Vagos</p>
            </div>
            <div className="bg-brand-white p-6 rounded-xl shadow-lg text-center">
                <p className="text-3xl sm:text-4xl font-bold text-brand-blue">{totalSlots}</p>
                <p className="font-semibold text-brand-gray">Total de Vagas</p>
            </div>
        </div>
      </div>
    </div>
